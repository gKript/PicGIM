...
//			}
	...
	_pg_Uint8	pg_adc_get_perc( void ) {
		return (_pg_Uint8)( ( (float)100 / ((float)PG_ADC_RES_STEPS - 1.00 )) * ((float)pg_adc_get() + 1.00) );
	}


	float	pg_adc_get_perc_f( void ) {
		return (float)( ( (float)100.0 / ((float)PG_ADC_RES_STEPS - 1.00 )) * ((float)pg_adc_get() + 1.00) );
	}
